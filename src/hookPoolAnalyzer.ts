...
export class HookPoolAnalyzer {
  ...
  async getPoolState(): Promise<PoolState> {
    ...
    return {
      ...
      pendingRebalance: pendingRebalance === 1n,
      rebalanceEnabled: rebalanceEnabled === 1n,
      referenceTick,
  ...
  async getSwapSummary(hours: number = 24): Promise<SwapSummary> {
    ...
    for (const l of logs) {
      ...
      if (buy) {
        buys++;
        // Buy: a0 < 0 (ETH into pool), a1 > 0 (IMD out of pool)
        buyEthIn += eth(-a0);
        buyImdOut += eth(a1);
      } else {
        sells++;
        // Sell: a0 > 0 (ETH out of pool), a1 < 0 (IMD into pool)
        sellEthOut += eth(a0);
        sellImdIn += eth(-a1);
      }
    }

    // Volume is ETH in + ETH out
    const volumeEth = buyEthIn + sellEthOut;
    const netEthFlow = buyEthIn - sellEthOut;
    ...
    return {
      ...
      volumeEth,
      netEthFlow,
      avgSwapSizeEth: logs.length > 0 ? volumeEth / logs.length : 0,
    };
  }
  ...
  async getVolumeShare(hours: number = 24): Promise<{ hookVolume: number; referenceVolume: number; share: number }> {
    ...
    // Get hook pool swaps
    const hookLogs = await this.ch.logs(
      CONFIG.contracts.poolManager,
      [TOPICS.Swap, this.poolId],
      from,
      to
    ...
    // Get reference pool swaps
    const refLogs = await this.ch.logs(
      CONFIG.contracts.poolManager,
      [TOPICS.Swap, CONFIG.contracts.referencePoolId],
      from,
      to
    );
    ...
    for (const l of hookLogs) {
      const a0 = signed(word(l.data, 0));
      hookVolume += eth(a0 < 0n ? -a0 : a0);
    }

    let referenceVolume = 0;
    for (const l of refLogs) {
      const a0 = signed(word(l.data, 0));
      referenceVolume += eth(a0 < 0n ? -a0 : a0);
    }
    ...
    return {
      hookVolume,
      referenceVolume,
      share: total > 0 ? hookVolume / total : 0,
    };
  }
